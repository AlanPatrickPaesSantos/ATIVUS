...
test('renders equipment metrics, situation summary and recent activity for the authenticated Unit', () => {
  ...
  expect(screen.queryByRole('heading', { name: 'Atenção operacional' })).not.toBeInTheDocument()
  expect(screen.queryByRole('heading', { name: 'Evolução das manutenções' })).not.toBeInTheDocument()
  expect(screen.queryByLabelText('Mai: 1 manutenção')).not.toBeInTheDocument()
