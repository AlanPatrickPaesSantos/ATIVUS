...
describe('report routes', () => {
  ...
  it('exports the authenticated inventory summary as a PDF document', async () => {
    ...
    expect(pdf).toContain('(PAINEL EXECUTIVO)');
    expect(pdf).toContain('(LEITURA ADMINISTRATIVA)');
    expect(pdf).toContain('(Confer\\352ncia administrativa)');
    expect(pdf).toContain('(Invent\\341rio consolidado)');
    expect(pdf).toContain('(Sistema de Gest\\343o Patrimonial)');
    expect(pdf).toContain('(Consolida\\347\\343o estadual)');
